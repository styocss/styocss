...
import { initPikaCSS, preparePikaCSS } from '@pikacss/integration'
...
/** Thin package-local CLI wrapper over Integration-owned init/prepare operations. */
export async function runCli(args: readonly string[], io: CliIO = defaultIO): Promise<number> {
	try {
		...
		if (command === 'init') {
			...
			io.stdout('Run `pikacss prepare` to publish PikaCSS generated authoring state.\n')
			return 0
		}
