...
== Package summary
== Functions (package root)
=== individual exported functions
== Constants (package root)
=== individual exported constants
== Types (package root)
=== individual interfaces and type aliases
== Classes (package root, when present)
=== individual exported classes
== Public subpath: `{package}/{subpath}` (one section for each non-root entry declared by the package `exports` map)
=== import guidance and, when applicable, a re-export note
=== entry-specific exported declarations (one heading per declaration)
== Module Augmentations (when present)
=== individual augmented interfaces
== Next
=== generated navigation links

The generator always documents the package root first. For each public
subpath, it renders only declarations whose source is specific to that entry;
entries that only re-export the root receive a short explanatory note. This
keeps API pages aligned with the package export map without maintaining a
package-specific subpath inventory here.
