...
- **`scripts/release.ts` exists for one check only: that the release starts from an up-to-date `main`.** `bumpp` already covers clean-tree, lockstep bump, confirmation, commit, tag, and branch-before-tag push order — do not reimplement any of that here. What `bumpp` does not check is the branch, and a tag pushed from a feature branch or a stale `main` fails `release.yml`'s ancestor check only after the tag is on origin, spending the version number.
