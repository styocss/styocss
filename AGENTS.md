...
- **Do not rename `release.yml`, and do not delete the `release` environment.** npm trusted publishing authorizes a specific repository *and workflow filename*, and may additionally be scoped to that environment; either change breaks the OIDC exchange with no local signal. The environment has no reviewer — merging the version pull request is the human gate — but it must keep existing, which is why it is absent from `UNUSED_ENVIRONMENTS` in `scripts/setup-repo-guardrails.sh`.
- **One GitHub rule shapes the whole release flow: nothing done with `GITHUB_TOKEN` starts another workflow run.** That is why the pull request is opened by hand (a bot-pushed branch gets no CI, so the required checks would never report and the merge would be blocked forever), why the tag is pushed by hand (a bot-pushed tag would not trigger `release.yml`), and why `deploy-docs` is chained with `needs:` instead of reacting to the tag. Any attempt to automate those steps has to solve this first — `workflow_dispatch` and `repository_dispatch` are the only exceptions to the rule.
