...
- Enforcement lives in exactly one place: GitHub branch protection, applied by `scripts/setup-repo-guardrails.sh` (`pnpm setup:guardrails`, emergency bypass documented in its header). It requires a pull request and seven green checks on `main`, and requires no review — a sole maintainer cannot approve their own pull request, so requiring one would deadlock every pull request. There is no `CODEOWNERS`; with zero required approvals it enforced nothing, and the paths worth pausing on are the ones listed under `ask` in `.claude/settings.json`.
- `.claude/settings.json` is a local checkpoint layer, not a security boundary — it prompts, and its patterns match command strings, so a different spelling can slip past.
- **`main` is protected server-side; everything else is a prompt.** Branch protection requires a pull request and seven green checks, blocks force pushes, and applies to admins, so no local rule can bypass it. Locally, only two things are hard-denied: publishing to npm, and writing generated outputs (`pika.gen.*`, `dist/`, `coverage/`). Everything else at most asks.
- **Merging and pushing stay the owner's decision, but an agent may execute them.** `git push`, `gh pr merge`, `gh workflow run`, and any state-changing `gh api` call prompt for approval every time. Do them when the owner asks for them, never as an inferred next step: open the pull request, report what needs deciding, and wait.
