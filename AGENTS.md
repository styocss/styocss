...
- **Releasing is two staged workflows and two owner actions.** `release-prepare.yml` (`workflow_dispatch`, choose the bump type) bumps every `package.json` on a `release/v*` branch and opens a pull request labelled `release`; the owner merges it; `release-publish.yml` reacts to that merge by tagging the merged commit, publishing with npm trusted publishing, writing the release notes, and redeploying the docs. Nothing pushes to `main` directly — that is why branch protection needs no bypass, and why the merge itself is the human gate rather than a separate environment approval. Two consequences worth knowing before editing these files: a pull request opened with `GITHUB_TOKEN` does not start a workflow run (hence `ci.yml` also triggers on `release/**` pushes, so the required checks report on the same commit), and a tag pushed with `GITHUB_TOKEN` does not either (hence repopack and docs deploy are chained with `needs:` instead of triggering on the tag).
