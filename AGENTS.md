...
- Enforcement lives in exactly one place: GitHub branch protection, applied by `scripts/setup-repo-guardrails.sh` (`pnpm setup:guardrails`, emergency bypass documented in its header). `.claude/settings.json` is a local checkpoint layer, not a security boundary — it prompts, and its patterns match command strings, so a different spelling can slip past. `.github/CODEOWNERS` documents which paths the owner cares about; it does not currently gate merges, because `required_approving_review_count` is 0.
