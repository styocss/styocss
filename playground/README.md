...
Deployed to `https://pikacss.github.io/playground/` by `.github/workflows/deploy-docs.yml`, which copies the built output into the docs dist under `playground/`. That workflow runs after a release publishes, or on manual dispatch — never on a push to `main`, so a playground change is live only once one of those happens. GitHub Pages cannot send the COOP/COEP headers WebContainers require, so `public/coi-serviceworker.min.js` installs a service worker that injects them (`index.html` loads it first).
